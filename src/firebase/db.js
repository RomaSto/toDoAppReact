...
export const deleteTodo = (userId, todoId) => db.ref(`users/${userId}/toDos/${todoId}`).remove();
...
export const push = (authUser, todo) => {
  ...
  db.ref(`users/${authUser.uid}/toDos`).push(todo);
...
export const updatePriority = (authUser, todos) => new Promise((resolve) => {
  db.ref(`users/${authUser.uid}/toDos`)
    .update(todos)
